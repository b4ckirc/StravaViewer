...
def render(tab, activity):
    ...
    if not has_hr:
        no_data(tab, "Nessun dato di frequenza cardiaca disponibile.")
        return

    ctrl = tk.Frame(tab, bg=C["surface"], pady=10)
    ctrl.pack(fill="x")
    tk.Label(ctrl, text="FC MASSIMA:", font=("Courier", 9, "bold"),
    ...
    tk.Button(ctrl, text="AGGIORNA", font=("Courier", 8, "bold"),
              bg=C["accent"], fg="white", bd=0, padx=10, pady=4, cursor="hand2",
              command=lambda: _draw(a, hr_max_var, holder)
              ).pack(side="left", padx=8)
    tk.Label(ctrl, text="(Modifica la FC max per ricalcolare le zone)",
             font=("Courier", 8), fg=C["text_dim"], bg=C["surface"]).pack(side="left")
    info_btn(ctrl, "Come leggere i grafici delle Zone HR", _INFO_HR).pack(
        side="right", padx=16)
...
def _draw(a, hr_max_var, holder):
    ...
    names  = [z[0] for z in HR_ZONES]
    colors = [z[3] for z in HR_ZONES]
    pcts   = [zone_secs[n] / total_secs * 100 for n in names]
    times  = [fmt_time(int(zone_secs[n])) for n in names]
    bars = ax1.barh(names, pcts, color=colors, alpha=0.85, height=0.55)
    ax1.set_xlim(0, 108)
    ax1.set_xlabel("% del tempo", fontsize=7)
    style_ax(ax1, "DISTRIBUZIONE ZONE CARDIACHE")
    ax1.invert_yaxis()
    for bar, pct, t in zip(bars, pcts, times):
        if pct > 1:
            ax1.text(pct + 1, bar.get_y() + bar.get_height() / 2,
                     f"{pct:.1f}%  ({t})", va="center",
                     fontsize=7, color=C["text"], fontfamily="monospace")
    ax1.text(0.99, 0.02, f"FC max: {hr_max:.0f} bpm",
    ...
    if valid:
            ...
            try:
                ...
                if d:
                    ...
                    ax2.plot(xr, [aa + b*x for x in xr], color=C["yellow"],
                             lw=1.5, linestyle="--", label="Tendenza", zorder=2)
        ...
        ax2.set_xlabel("Frequenza cardiaca (bpm)", fontsize=7)
        ax2.set_ylabel("Passo (min/km)", fontsize=7)
        ax2.yaxis.set_major_formatter(FuncFormatter(
            lambda y, _: pace_label(y) if y > 0 else ""))
        handles = [mpatches.Patch(color=z[3], label=z[0]) for z in HR_ZONES]
        ax2.legend(handles=handles, fontsize=6.5, facecolor=C["surface2"],
                   edgecolor=C["border"], labelcolor=C["text"],
                   loc="upper right", title="Zone HR", title_fontsize=6)
    else:
        ax2.text(0.5, 0.5, "Dati insufficienti", ha="center", va="center",
                 color=C["text_dim"], transform=ax2.transAxes)
    style_ax(ax2, "SCATTER: FC vs PASSO")

    fig.suptitle(f"Analisi zone cardiache  •  {a.name}",
